	...
	public NatTraverserSimulationMain() throws IOException {
		...
		Component simulator = create(NatTraverserSimulator.class);
                ...
                final NatTraverserConfiguration ntConfig = 
                        (NatTraverserConfiguration)
                        AbstractConfiguration.load(NatTraverserConfiguration.class);
                final HpClientConfiguration hpClientConfig = (HpClientConfiguration)
                        HpClientConfiguration.load(HpClientConfiguration.class);
		final ParentMakerConfiguration parentMakerConfiguration = 
                ...
                final NatConfiguration natConfiguration = 
                        (NatConfiguration)
                        NatConfiguration.load(NatConfiguration.class);
                final StunServerConfiguration stunClientConfig =
                        (StunServerConfiguration)
                        StunServerConfiguration.load(StunServerConfiguration.class);
                final RendezvousServerConfiguration rendezvousServerConfig =
                        (RendezvousServerConfiguration)
                        RendezvousServerConfiguration.load(RendezvousServerConfiguration.class);
		...
		trigger(new NatTraverserSimulatorInit(ntConfig, hpClientConfig,
                        rendezvousServerConfig, stunClientConfig,
                        parentMakerConfiguration, natConfiguration), simulator.getControl());
