...
public class PD_PC_PD_Test
    ...
    private static final Logger logger =  LoggerFactory.getLogger(PD_PC_PD_Test.class);
    /**
     * Create the test case
     *
     * @param testName name of the test case
     */
    public PD_PC_PD_Test(String testName) {
        super(testName);
    }
    ...
    public static Test suite() {
        return new TestSuite(PD_PC_PD_Test.class);
    }

    public static void setTestObj(PD_PC_PD_Test testObj) {
    ...
    public static class TestEmulatorComponent extends ComponentDefinition {
        ...
        private static PD_PC_PD_Test testObj = null;
        ...
        private Handler<TestMsgId> handleUpperMessage = new Handler<TestMsgId>() {
            ...
            public void handle(TestMsgId event)
            {
                ...
                if ((event.getSource().getIp().equals(dest1) && (event.getSource().getPort() == dest1Port))
                        || (event.getSource().getIp().equals(dest2) && (event.getSource().getPort() == dest2Port)))
                {
                    incomingMsgs--;
                    if (incomingMsgs == 0) {
                        testObj.pass();
        ...
        private Handler<TestMsgId> handleLowerMessage = new Handler<TestMsgId>() {
            ...
            public void handle(TestMsgId event) {
                        ...
                        && event.getSource().getIp().equals(natIP))
                {
                    ...
                    incomingMsgs--;
                    if(secondOpenedPort > firstOpenedPort)
                    {
                        ...
                        trigger(new TestMsgId(ToVodAddr.systemAddr(new Address(dest2, dest2Port+1, dest2Id)), ToVodAddr.systemAddr(new Address(natIP, secondOpenedPort, 100)), null/*not suppost to get in*/), emulator.getNegative(VodNetwork.class));
                        // this should not get in too
                        ...
                        // this should get in
                        trigger(new TestMsgId(ToVodAddr.systemAddr(new Address(dest1, dest1Port, dest1Id)), ToVodAddr.systemAddr(new Address(natIP, firstOpenedPort, 100)), msgTimeout1.getTimeoutId()), emulator.getNegative(VodNetwork.class));
                        // this should also get in
                        trigger(new TestMsgId(ToVodAddr.systemAddr(new Address(dest2, dest2Port, dest2Id)), ToVodAddr.systemAddr(new Address(natIP, secondOpenedPort, 100)), msgTimeout2.getTimeoutId()), emulator.getNegative(VodNetwork.class));
    ...
    public void testApp() {
        ...
        try {
            PD_PC_PD_Test.semaphore.acquire(EVENT_COUNT);
