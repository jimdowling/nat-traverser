 ...
 */
public class EI_PP_HD_Test_ToBeCompleted
        extends TestCase {
    ...
    private static final Logger logger = LoggerFactory.getLogger(EI_PP_HD_Test_ToBeCompleted.class);
     ...
     */
    public EI_PP_HD_Test_ToBeCompleted(String testName) {
        super(testName);
    }
    ...
    public static Test suite() {
        return new TestSuite(EI_PP_HD_Test_ToBeCompleted.class);
    }

    public static void setTestObj(EI_PP_HD_Test_ToBeCompleted testObj) {
    ...
    public static class TestEmulatorComponent extends ComponentDefinition {
        ...
        private static EI_PP_HD_Test_ToBeCompleted testObj = null;
        ...
        public Handler<Start> handleStart = new Handler<Start>() {
            ...
            public void handle(Start event) {
                ...
                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(src1, src1Port, src1Id)), 
                        ToVodAddr.systemAddr(new Address(dest1, dest1Port, dest1Id)), 
                        msgTimeout1.getTimeoutId()), 
                        emulator.getPositive(VodNetwork.class));
                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(src1, src1Port, src1Id)), 
                        ToVodAddr.systemAddr(new Address(dest2, dest2Port, dest2Id)), 
                        msgTimeout2.getTimeoutId()), emulator.getPositive(VodNetwork.class));
                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(src1, src1Port+1, src1Id)), 
                        ToVodAddr.systemAddr(new Address(dest2, dest2Port, dest2Id)), 
                        msgTimeout2.getTimeoutId()), 
                        emulator.getPositive(VodNetwork.class));

                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(dest1, dest1Port, dest1Id)), 
                        ToVodAddr.systemAddr(new Address(natIP, src1Port, natId)), 
                        msgTimeout3.getTimeoutId()), emulator.getNegative(VodNetwork.class));
                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(dest2, dest2Port, dest2Id)), 
                        ToVodAddr.systemAddr(new Address(natIP, src1Port, natId)), 
                        msgTimeout4.getTimeoutId()), emulator.getNegative(VodNetwork.class));
                trigger(new TestMsgId(
                        ToVodAddr.systemAddr(new Address(dest2, dest2Port+1, dest2Id)), 
                        ToVodAddr.systemAddr(new Address(natIP, src1Port, natId)), 
                        msgTimeout4.getTimeoutId()), emulator.getNegative(VodNetwork.class));
        ...
        private Handler<TestMsgId> handleUpperMessage = new Handler<TestMsgId>() {
            ...
            public void handle(TestMsgId event) {
                logger.debug("JTest Upper Message Handler timer id " + event.getTimeoutId());
                trigger(new CancelTimeout(event.getTimeoutId()), timer.getPositive(Timer.class));
                logger.debug("msg recvd src: "+event.getSource()+" dest "+event.getDestination());
            }
        };
        private Handler<TestMsgId> handleLowerMessage = new Handler<TestMsgId>() {
            ...
            public void handle(TestMsgId event) {
                ...
                logger.debug(event.getSource().getIp() + " " + event.getSource().getPort());
    ...
    public void testApp() {
        ...
        try {
            EI_PP_HD_Test_ToBeCompleted.semaphore.acquire(EVENT_COUNT);
