...
//    HashSet<Address> registeredServers = new HashSet<Address>(); // servers with whome the client is registered
     ...
     */
    private int msgRetryDelay;
    private int numMsgRetries;
    private double msgRetryRTO = 1.5d;
...
//
    ...
    Handler<HpClientInit> handleInit = new Handler<HpClientInit>() {
        ...
        public void handle(HpClientInit init) {
            ...
            msgRetryDelay = init.getConfig().getMessageRetryDelay();
                ...
                if (!remoteAddr.hasParents()) {
                    ...
                    if (!hpSessions.containsKey(remoteId)) {
                        HpSession session = new HpSession(
                                ...
                                request, scanRetries, scanningEnabled,
        ...
        if (hp != null) {
            ...
            if (hp.getClient_A_HPRole() == HPRole.PRP_INITIATOR) {
                ...
                ScheduleRetryTimeout st =
                        new ScheduleRetryTimeout(msgRetryDelay, numMsgRetries, msgRetryRTO);
    ...
    Handler<DeleteConnectionRequest> handleDeleteConnectionRequest = new Handler<DeleteConnectionRequest>() {
        ...
        public void handle(DeleteConnectionRequest request) {
            ...
            logger.trace(compName + " deleting the connection " + request.getConnectionKey());

            // step 1
//            dontKeepConnectionOpen(request.getConnectionKey());

            // step 2
            OpenedConnection openedConnection = openedConnections.get(request.getConnectionKey());
            ...
            if (openedConnection != null) {
                ...
                // Generate a random msgTimeoutId for this request.
                ...
                // step 3
                openedConnections.remove(request.getConnectionKey());
            }
        ...
        public void handle(SHP_OpenHoleMsg.Initiator response) {
            ...
            if (response.getResponseType() != SHP_OpenHoleMsg.ResponseType.OK) {
                ...
                sendHolePunchingMsg(self.getAddress(), session.getRemoteOpenedHole(),
                        response.getMsgTimeoutId(),
                        response.getVodSource().getId(), 0, 1000, 1d);
            }
        }
    };
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private void sendHolePunchingMsg(VodAddress sourceAddress, VodAddress dest,
            TimeoutId msgTimeoutId,
            int zServerId, int msgRetries, int msgRetryDelay, double scaleRetries) {
...
//        }
        ...
        ScheduleRetryTimeout st =
                new ScheduleRetryTimeout(msgRetryDelay, msgRetries, scaleRetries);
...
//                        : self.getPort();
                ...
                HolePunchingMsg.Response hpResponse = new HolePunchingMsg.Response(sourceAddress,
                        ...
                        request.getTimeoutId());
                ScheduleRetryTimeout st = new ScheduleRetryTimeout(msgRetryDelay,
                        numMsgRetries, msgRetryRTO);
                ...
                if (req != null) {
                    ...
                    logger.debug(compName + "no need to send response to the upper component");
                }
            ...
            if (delegator.doCancelRetry(response.getTimeoutId())) {
                ...
                if (session != null) {
                    ...
                    if (!openedConnections.containsKey(remoteId)) {
                        ...
                        addOpenedConnection(openedHole, session.getZServers().iterator().next(),
                                session);

                        logger.debug(compName + "Hole session registered f("
                                + self.getId() + ","
                                + remoteId
                                + ") - received msg at "
...
//                    hpSessions.remove(remoteId);
    ...
    private void addOpenedConnection(VodAddress openedHole, Address zServer, HpSession session) {
        ...
        if (session.isHeartbeatConnection()) {
            ...
            openedConnection = new OpenedConnection(
                    ToVodAddr.hpServer(zServer),
                    session.getHolePunchingMechanism(),
                    session.getHolePunchingRole(),
                    session.getPortInUse(),
                    openedHole.getPeerAddress(),
                    session.getDummyAddress(),
                    natBindingTimeout,
    ...
    Handler<HpKeepAliveMsg.Pong> handleHpKeepAliveMsgPong =
        ...
        public void handle(HpKeepAliveMsg.Pong event) {
            ...
            if (oc == null) {
                ...
                st.setTimeoutEvent(sht);
                trigger(st, timer);
                oc.setHbTimeoutId(sht.getTimeoutId());
                updateLastHeardFrom(event.getSource().getId());
            }
        }
    };
    Handler<SendHeartbeatTimeout> handleSendHeartbeatTimeout =
        ...
        public void handle(SendHeartbeatTimeout event) {
            ...
            if (oc == null) {
                ...
                // set the timeout for 5 seconds less than the minimum NAT binding time for the 2 NATs
                ScheduleRetryTimeout srt = new ScheduleRetryTimeout(hbTime - 5, 2, 0.5);
                HeartbeatTimeout hbt = new HeartbeatTimeout(srt, pingMsg);
                TimeoutId timeoutId = delegator.doRetry(hbt);
                delegator.doRetry(hbt);
                oc.setHbTimeoutId(timeoutId);
            }
    ...
    Handler<HeartbeatTimeout> handleHeartbeatTimeout =
        ...
        public void handle(HeartbeatTimeout event) {
            ...
            OpenedConnection deleteOc = null;
            for (OpenedConnection oc : openedConnections.values()) {
                if (oc.getHbTimeoutId().equals(oc)) {
                    deleteOc = oc;
                }
            }
            if (deleteOc != null) {
                openedConnections.remove(deleteOc.getHoleOpened().getId());
                logger.debug(compName + " heartbeat timed out and removing " + id);
            } else {
                logger.error(compName + " couldn't find connection to delete for timeoutId"
                        + id);
            }
        }
    };
        ...
        public void handle(HolePunchingMsg.ResponseAck msg) {
            ...
            if (delegator.doCancelRetry(msg.getTimeoutId())) {
                ...
                if (session.getOpenConnectionRequest() != null) {
                    ...
                    logger.debug(compName + "not sending response to the outer component");
                    // no need to send response to the upper component
                }
...
//                        request.getNumRetries(), 1000, 1.2d);
//                return;
//            } else 
...
//            } else if ((session.getHolePunchingMechanism() == HPMechanism.PRP_PRP && newSession == true)) {
//                bindFirst = true;
//                portToBeUsed = request.get_PRP_PRP_InterleavedPort();
            ...
            if (bindFirst) {
                ...
                GoMsg_PortResponse bindResp = new GoMsg_PortResponse(bindReq, remoteId,
                        request.getSource().getId(), request.getNumRetries(),
                        request.isBindPort(),
                        request.getMsgTimeoutId());
                ...
                prepareAndSendHPMessage(portToBeUsed, remoteId,
                        request.getVodSource().getId(),
                        request.getNumRetries(), request.getMsgTimeoutId());
            }
        }
    };
...
//                            HpSession session = hpSessions.get(response.getKey());
    ...
    private void prepareAndSendHPMessage(int srcPort, Integer sessionKey, int zServerId,
            int numRetries, TimeoutId msgTimeoutId) {
        // sanity check
        ...
        int rto = 1000;
        double scaleRto = 2.0d;
        if (session.getHolePunchingMechanism() == HPMechanism.PRC_PRC) {
            numRetries = numMsgRetries + scanRetries * (numMsgRetries + 1) + 2 /*
            ...
            logger.debug(compName + "possible situation where there has to be a lot"
                    ...
                    + ") total retries " + numRetries);
            rto = 500;
            scaleRto = 1.1d;
        } else {
//            numRetries = messageRetries;
        }

        sendHolePunchingMsg(newSourceAddress, session.getRemoteOpenedHole(),
                msgTimeoutId, zServerId, numRetries, rto, scaleRto);
    }
...
//                    }                
    ...
    private void sendPrpPortsToServer(Set<Integer> someAvailablePorts,
        ...
        ScheduleRetryTimeout st = new ScheduleRetryTimeout(msgRetryDelay, numMsgRetries, msgRetryRTO);
