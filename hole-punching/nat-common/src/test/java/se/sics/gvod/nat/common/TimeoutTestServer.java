 ...
 */
public final class TimeoutTestServer extends AutoSubscribeComponent {

    private Negative<VodNetwork> net = negative(VodNetwork.class);
    private long startTime = System.currentTimeMillis();

    public TimeoutTestServer() {
        autoSubscribe();
    }
    ...
    private Handler<TMessage.RequestMsg> handleTMessage = new Handler<TMessage.RequestMsg>() {
        ...
        public void handle(TMessage.RequestMsg event)
        {
            ...
            TMessage.ResponseMsg response = new TMessage.ResponseMsg(event.getVodDestination(), 
                    event.getVodSource(), event.getTimeoutId());
