...
public class BDecoder {
    ...
    public BEValue bdecode() throws IOException {
        ...
        if (indicator >= '0' && indicator <= '9') {
            ...
            return bdecodeAddress();
        } else if (indicator ==  'b') {
            return bdecodeBooleans();
        } else {
    ...
    public BEValue bdecodeBooleans() throws IOException {
        int c = getNextIndicator();
        if (c!='b'){
            throw new InvalidBEncodingException("Expected 'b', not '" + (char) c
                    + "'");
        }
        indicator = 0;
        c=getNextIndicator();
        List<BEValue> l = bdecode().getList();
        boolean[] bs = new boolean[l.size()];
        for(int i=0;i<l.size();i++){
            bs[i]=Boolean.parseBoolean(l.get(i).getString());
        }
        return new BEValue(bs);
    }
    /**
    ...
    public BEValue bdecodeAddress() throws IOException {
        int c = getNextIndicator();
        if (c != 'a') {
            throw new InvalidBEncodingException("Expected 'a', not '" + (char) c
                    + "'");
        }
        indicator = 0;
        c = getNextIndicator();
        int id = bdecode().getInt();
        c = getNextIndicator();
        char[] ch = bdecode().getString().toCharArray();
        String s;
        if (ch[0]=='/'){
            char[] cha = new char[ch.length-1];
        ...
        c = getNextIndicator();
        int port = bdecode().getInt();

        Address result = new Address(ip, port, id);
        c = getNextIndicator();
        if (c!='e'){
            throw new InvalidBEncodingException("Address should end with 'e'");
        }
        indicator = 0;
        return new BEValue(result);
    }
